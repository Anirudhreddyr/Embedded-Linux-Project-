/* TFTP UDP Client code RRQ + WRQ */
#include <stdio.h>              // Standard I/O functions (e.g., printf, fprintf)
#include <stdlib.h>             // For general utilities (e.g., exit, malloc, free)
#include <string.h>             // For string operations (e.g., memset, strcpy, strlen)
#include <unistd.h>             // For close() and other POSIX APIs
#include <sys/socket.h>         // For socket APIs (e.g., socket, sendto, recvfrom)
#include <netinet/in.h>         // For sockaddr_in structure and INADDR_ANY
#include <arpa/inet.h>          // For address conversion (e.g., htons, inet_addr)

#define PORT 69                 // TFTP default port number (standard as per RFC 1350)
#define SERVER_IP "127.0.0.1"   // IP address of the TFTP server (localhost for testing)

#define BUFFER_SIZE 516         // Total TFTP packet size: 4 bytes header + 512 bytes data
#define BLOCK_SIZE 512          // Size of each data block transferred in TFTP

/* TFTP Opcodes as defined by RFC 1350 */
#define RRQ 1                   // Read Request (client wants to read/download a file)
#define WRQ 2                   // Write Request (client wants to write/upload a file)
#define DATA 3                  // Data Packet (used to send file data)
#define ACK 4                   // Acknowledgment Packet (used to confirm receipt of data)
#define ERROR 5                 // Error Packet (used to notify issues like file not found, etc.)

#define MODE "octet"            // Transfer mode - "octet" means binary (raw) transfer mode
